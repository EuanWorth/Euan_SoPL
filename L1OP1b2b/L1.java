package L1OP1b2b;//Matthew Parkinson, 1/2004
...
class State {
    java.util.HashMap<Location, Value> store = new java.util.HashMap<>();
    ...
    public String toString() {
        ...
        Iterator<Map.Entry<Location, Value>> iter = store.entrySet().iterator();
        while (iter.hasNext()) {
            Map.Entry<Location, Value> e = iter.next();
            ret.append("(").append(e.getKey()).append(" |-> ").append(e.getValue()).append(")");
            if (iter.hasNext()) ret.append(", ");
        }
        return ret + "]";
    }
}
...
class Environment {
    java.util.HashSet<Location> env = new java.util.HashSet<>();
...
class CanNotReduce extends Exception {
    CanNotReduce(String reason) {
        super(reason);
    }
}

class TypeError extends Exception {
    TypeError(String reason) {
        super(reason);
    }
}
